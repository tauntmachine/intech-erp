...
import CustomDropdownEditor from "./CustomDropdownRenderer";
import DeleteButtonModal from "../components/Modals/DeleteButtonModal";
import ClearableProp from "../../src/components/Inputs/CalenderInput";
import CustomDatePicker from "../components/Inputs/CustomDatePicker";
import AntDesignDatePicker from "../components/Inputs/AntDesignDatePicker";
...
import { Currency } from "styled-icons/remix-fill";
...
const FlexContainer = styled.div`
  ...
  /* margin-top: 30px; */
  /* background-color: green; */
  width: 11rem; /* Adjust gap as needed */
`;
...
const SupplierPayNew = (props) => {
  const [selectedDate, setSelectedDate] = useState(null);

  const handleDateChange = (date, dateString) => {
    setSelectedDate(dateString);
    console.log(dateString); // Log the selected date
  };
  ...
  const handleDropdownChange = (selectedOption) => {
    console.log("Selected option in parent component:", selectedOption);
  };
  ...
  const handleAddRowAbove = () => {
    setRowData1((prevRowData) => {
      const selectedRowIndex = prevRowData.findIndex(
        (row) => row.id === selectedRowId
      );
      const newRow = createNewRow();
      const updatedRowData = [...prevRowData];
      updatedRowData.splice(selectedRowIndex, 0, newRow);
  ...
  const handleAddRowBelow = () => {
    setRowData1((prevRowData) => {
      ...
      const newRow = createNewRow();
      const updatedRowData = [...prevRowData];
      updatedRowData.splice(selectedRowIndex + 1, 0, newRow);
      return updatedRowData;
    });
    setShowDeleteModal(false);
  };
  ...
  const colDefs1 = [
    {
      ...
      sortable: false,
      cellStyle: (params) => ({
        outline: params.node.isSelected() ? "none" : undefined,
        backgroundColor: params.node.isSelected() ? "transparent" : undefined,
        border: "none",
      }),
      resizable: false,
      cellRenderer: (params) =>
        params.node.rowIndex === rowData1.length - 1 ? null : (
          <div>
            {selectedRowId === params.data.id && showDeleteModal && (
              <DeleteButtonModal
                show={selectedRowId === params.data.id && showDeleteModal}
            ...
            <Dotscell onClick={() => handleDotsClick(params.data.id)} />
          </div>
        ),
    },
    {
    ...
    },
    {
      ...
      cellEditorParams: {
        values: [
          "Lenovo Thinkpad 100BA-14",
          "PC Pentium 4",
          "Laptop Dell i5 7 Generation",
          "IPad 6",
        ],
      },
      cellRenderer: (params) => (
        <CustomerSupplierCell value={params.value} lineText={params.value} />
      ),
      comparator: customSortComparator,
    },
    {
    ...
    },
    {
      ...
      cellEditorParams: {
        values: [
          ...
          "Warehouse2 Abudhabi",
          "Warehouse3 Sharjah",
          "Warehouse4 Expo Center",
          "Warehouse5 Dubai Stadium",
        ],
      },
      comparator: customSortComparator,
    },
  ...
  const onSortChanged = (params) => {
    ...
    const addNewRowNode = sortedNodes.find(
      (node) => node.data.id === "addNewRow"
    );
    if (addNewRowNode) {
      sortedNodes.splice(sortedNodes.indexOf(addNewRowNode), 1);
      sortedNodes.push(addNewRowNode);
    }
    params.api.setRowData(sortedNodes.map((node) => node.data));
  };
  ...
  const [colDefs7, setColDefs7] = useState([
    {
      filed: "dot",
      headerName: null,
      headerClass: "All-header",
  ...
  //   'last-row': (params) => params.node.rowIndex === rowData1.length - 1,
  // };
  // const lastRowCellRenderer = (params) => {
  //   if (params.node.rowIndex === rowData1.length - 1) {
  //     return (
  //       <AddRowButton onClick={() => alert('Add Row clicked')} />
  //     );
  //   }
  //   return params.value;
  // };
      ...
      <Main>
        <Wrapit>
          <Header
            ...
            fourthNav={"Supplier Payment List"}
            navActive={"Supplier Payment - New"}
          />
          <HeaderNewButtons />
          <Div>
            {/* <AccountsSideNav
            ...
            /> */}
            <Hero>
              <TitleOfSection title={"SUPPLIER PAYMENT DETAILS"} />
              ...
              <Wrapper>
                ...
                <LeftWrapper>
                  ...
                  {/* <FieldsBox>
                <AntDesignDatePicker onChange={handleDateChange} label="Date"/>
                ...
                <FieldsBox>
                  <ChartAccountInput
                    Name="Payment Voucher No."
                    image={Auto}
                    setWidth={"long"}
                  />
                  <ChartAccountInput
                    Name="Extra Reference No."
                    setWidth={"long"}
                  />
                  {/* <CustomDatePicker Title="Transaction Date"/>
                  <CustomDatePicker Title="Posting Date"/>
                  <CustomDatePicker Title="Validity Date"/> */}

                  <CalanderInput Name={"Transaction Date"} setWidth={"long"} />
                  <CalanderInput Name={"Posting Date"} setWidth={"long"} />
                  <CalanderInput Name={"Due Date"} setWidth={"long"} />
                </FieldsBox>
                {/* </Inputs> */}
              </Wrapper>
              <BottomRowInputs>
                ...
                <DropdownInput
                  ...
                  setWidth={"LittleLarge"}
                />
                {/* <TagsInput show={false} setHeight={true} /> */}
                <FlexContainer>
                  <Text3 txtColor={"#58606f"}>Advance Payment</Text3>
                  <InputCheckOne
