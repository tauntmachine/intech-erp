...
const Btn = styled.div`
  padding: ${(props) => (props.height ? "7px 10px" : "7px 10px")};
  border-radius: 8px;
...
const MRTToggleButton = ({ 
  ...
  fixWidth, 
  size,
  toggleVisibility // New prop for the visibility function
}) => {
  const sBlue = 'blue';
  const sGrey = 'grey';
  ...
  const handleToggle = () => {
    ...
    if (toggleVisibility) { // Call the visibility function if provided
      toggleVisibility();
    }
  };
  ...
  return (
    ...
    >
      <IconContainer>
        {isOpen ? (
          <FaAngleRight color="#fff" size={13.5} />
        ) : (
          <FaAngleDown color="#fff" size={13.5} />
        )}
      </IconContainer>
...
export default MRTToggleButton;
